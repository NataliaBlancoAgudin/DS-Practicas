...
import model.foto.AdaptadorFoto;
import model.foto.Foto;
import model.monumento.AdaptadorMonumento;
import model.monumento.Monumento;
import model.restaurante.AdaptadorRestaurante;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (Monumento monumento : db.selectMonumentos()) {
            AdaptadorMonumento adaptadorMonumento = new AdaptadorMonumento(monumento, navegador);
            map.add(adaptadorMonumento);
        }

        for (Foto foto : db.selectFotos()) {
            AdaptadorFoto adaptadorFoto = new AdaptadorFoto(foto);
            map.add(adaptadorFoto);
        }

        for (Restaurante restaurante : db.selectRestaurantes()) {
            AdaptadorRestaurante adaptadorRestaurante = new AdaptadorRestaurante(restaurante, navegador);
            map.add(adaptadorRestaurante);
        }
